...
export default class Gestor_inspeccionar{
    ...
    static async Analizar_datos_inspeccion(req,res,data){
        ...
        if(activa){
            ...
            req.flash("mensaje","No es posible registrar defectos de reproceso ya que la orden de produccion a la que te asociaste ha finalizado");
            res.redirect('/nav/menu_supervisor_calidad');
        }
    }
    ...
    static async Obtener_datos_incidencia(req,data){
        const {defecto,pie,hora} = req.params, {Id_horario_control} = data;
        const result = await Ejecutar_consulta("select Id_incidencia,Cantidad from incidencia where Fecha = current_date() and Hora = '"+(hora+":00")+"' and Pie = '"+pie+"' and Estado = 'Pendiente' and Defecto = '"+defecto+"' and Id_horario_control = '"+Id_horario_control+"' ");
        return result;
    }
    ...
    static async Actualizar(req,res){
        ...
        const result_defectos = await Ejecutar_consulta("select Descripcion from defecto where Tipo_defecto = 'Reproceso' ");
        ...
        res.render('inspeccionar/reproceso/index',{
