...
export default class Gestor_pares_segunda{
    static async Obtener_orden_de_produccion(req,res){
        ...
        if(result.length!=0){
            await this.Verificar_posibilidad_de_registrar_pares_de_primera(req,res,result[0].Orden_de_produccion);
        }else{
    ...
    static async Verificar_posibilidad_de_registrar_pares_de_primera(req,res,op){
        let dni_empleado = req.session.usuario.empleado.dni;
        ...
        if(result.length!=0){
            ...
            res.render('pares_primera/index', { orden_de_produccion });
        }else{
            req.flash("mensaje","No es posible registrar pares de primera ya que la orden de produccion a la que te asociaste ha finalizado");
            res.redirect('/nav/menu_supervisor_calidad');
        }
    }
    ...
    static async Registrar_pares_primera(req,res){
        ...
        if(result.length!=0){
            ...
            if(!alerta){
                await this.Verificar_cantidad_ingresada_de_pares_de_primera(req,res,dni_empleado,cantidad,result[0].Orden_de_produccion);
            }else{
                req.flash("mensaje","No podes registrar pares de primera si el semaforo esta en rojo");
                res.redirect('/nav/menu_supervisor_calidad');
            }
        }
    }
    static async Verificar_cantidad_ingresada_de_pares_de_primera(req,res,dni_empleado,cantidad,op){
        if(cantidad!=''){
            const result = await this.Verificar_estado_orden_de_produccion(dni_empleado);
            if(result.length!=0){
                await Ejecutar_consulta("update orden_de_produccion set Pares_de_primera = '"+cantidad+"' where Id_orden_de_produccion = '"+op+"' ");
                req.flash("mensaje","Pares de primera registrados con exito");
                res.redirect('/pares_primera');
            }else{
                req.flash("mensaje","No es posible registrar pares de primera ya que la orden de produccion a la que te asociaste ha finalizado");
                res.redirect('/nav/menu_supervisor_calidad');
            }
